<a href="/journal/{{ $journal->slug }}"
    class="flex block gap-4 md:hover:bg-zinc-800 rounded-xl py-2 pr-2 md:p-2 transition md:hover:scale-[0.99]">

    @if($journal->image)
        <div class="flex flex-col">
            <img src="{{ asset('storage/' . $journal->image) }}"
                class="w-24 h-32 md:w-24 md:h-20 rounded-lg object-cover object-center flex-shrink-0">
